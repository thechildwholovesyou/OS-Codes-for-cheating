...
int main()
{
    ...
    int n; // enter no of cylinders to enter into request queue
    scanf("%d",&n);

    int request_queue[n];
    int seek_sequence[n];

    // enter cylinder no in req queuee
    for(int i=0;i<n;i++)
    {
        scanf("%d",&request_queue[i]);
    }
    // enter initial pos of RW head
    int initial_pos;
    scanf("%d",&initial_pos);

    bool vis[n]; // keep track of vis cylinders
